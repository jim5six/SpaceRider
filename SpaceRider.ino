...
void ShowShootAgainLamps()
{
    ...
    if ((BallFirstSwitchHitTime == 0 && BallSaveNumSeconds) || (BallSaveEndTime && CurrentTime < BallSaveEndTime))
    {
        ...
        if (BallSaveEndTime != 0)
            msRemaining = BallSaveEndTime - CurrentTime;
    ...
    else if (animationType == DISPLAY_OVERRIDE_ANIMATION_FLYBY)
    {
        ...
        if (overrideAnimationSeed != LastAnimationSeed[displayNum])
        {
            ...
            if (overrideAnimationSeed < AnimationStartSeed[displayNum])
                realAnimationSeed = (255 - AnimationStartSeed[displayNum]) + overrideAnimationSeed;
...
void PlaySoundEffect(unsigned int soundEffectNum)
{
    ...
    if (MachineState == MACHINE_STATE_INIT_GAMEPLAY)
        return;
...
void QueueNotification(unsigned int soundEffectNum, byte priority)
{
    if (CalloutsVolume == 0)
        return;
    if (SoundSelector < 3 || SoundSelector == 4 || SoundSelector == 7 || SoundSelector == 9)
        return;
    if (soundEffectNum < SOUND_EFFECT_BALL_SAVE || soundEffectNum >= (SOUND_EFFECT_BALL_SAVE + NUM_VOICE_NOTIFICATIONS))
        return;
...
int ManageGameMode()
{
        ...
        if (CurrentTime < SuperBlastOffEndTime)
        {
            for (byte count = 0; count < 4; count++)
            {
                if (count != CurrentPlayer)
                    OverrideScoreDisplay(count, (SuperBlastOffEndTime - CurrentTime) / 1000, DISPLAY_OVERRIDE_ANIMATION_FLUTTER);
            }
        }
    ...
    if (IsSuperSpinnerActive(CurrentTime))
    {
        ...
        {
            if (count != CurrentPlayer)
                OverrideScoreDisplay(count, (SuperSpinnerEndTime - CurrentTime) / 1000, DISPLAY_OVERRIDE_ANIMATION_FLUTTER);
        }
    }
    ...
    if (IsSuperPopsActive(CurrentTime))
    {
        ...
        {
            if (count != CurrentPlayer)
                OverrideScoreDisplay(count, (SuperPopEndTime - CurrentTime) / 1000, DISPLAY_OVERRIDE_ANIMATION_FLUTTER);
        }
    }
    else
    {
    ...
    if ((BallFirstSwitchHitTime == 0) && GoalsDisplayValue(Goals[CurrentPlayer]))
    { // If ball not in play and if any goals have been reached
        for (byte count = 0; count < 4; count++)
        {
            if (count != CurrentPlayer)
            {
...
int CountdownBonus(boolean curStateChanged)
{
    ...
    if (CountdownBonusHurryUp && countdownDelayTime > ((unsigned long)CountDownDelayTimes[9]))
...
void HandleGamePlaySwitches(byte switchHit)
{
    ...
    case SW_R_INLANE:
        ...
        {
            CurrentScores[CurrentPlayer] += 20000 * PlayfieldMultiplier[CurrentPlayer];
            RPU_SetLampState(LAMP_BONUS_200, 0, 0, 0);
        }
        else if (RPU_ReadLampState(LAMP_BONUS_300))
        {
            CurrentScores[CurrentPlayer] += 30000 * PlayfieldMultiplier[CurrentPlayer];
            RPU_SetLampState(LAMP_BONUS_300, 0, 0, 0);
        }
        else if (RPU_ReadLampState(LAMP_BONUS_400))
        {
            CurrentScores[CurrentPlayer] += 40000 * PlayfieldMultiplier[CurrentPlayer];
            RPU_SetLampState(LAMP_BONUS_400, 0, 0, 0);
        }
        else
        {
...
{ // Set goal flag and update display score
